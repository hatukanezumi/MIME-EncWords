...
(L<MIME::Charset::JA_JP/USE_ENCODE> 参照) は、
以下のオプションはなんの効果も持たない。
...
初期値は C<"YES"> で、最小限のエンコードをする。
また、C<"DISPNAME"> を指定すると、RFC5322 のアドレス仕様 (3.4節)
で述べている特殊文字を含む部分もエンコードする。
